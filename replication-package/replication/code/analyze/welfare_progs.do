...
* Purpose: command for creating Table 3
/******************************/
cap pr drop welfare_percentiles
pr define welfare_percentiles, rclass 
  ...
  foreach prog in snap ssi wic medicaid liheap ha tanf schoolmeals {
    ...
    * for negishi weights use the main dataset     
    if mi("`negishi'") use "$dir/data/tmp/emp_welfare_`weighttype'`nm'_`prog'", clear
    if !mi("`negishi'") use "$dir/data/tmp/emp_welfare_crramain_`prog'", clear  
  ...
  /***********************************************/
    ...
    ***** self-targeting term *******
        ...
        * redistribution reg coefficient 
        reghdfe redist_outcome `prog' [aw=wtfam_`prog'], absorb(rd_rk_c)
        gen mean_within_neut_`prog' = -_b[`prog'] / mean_smmu_overall * sigma_sq_`prog'    

        * insurance component: main less redistribution
        gen mean_within_ins_`prog' = mean_within_`prog' - mean_within_neut_`prog'
    ...
    egen cut_rk_c = cut(rd_rk_c), at(1 6 11 16 21 26 31 56 101)
      ...
      * get slopes of benefit and takeup rates
        preserve 
         ...
         drop *tmp
         export delimited using figures/welfare_ls_ingredients_`prog'.csv, replace 
         tempfile slopes
         save `slopes' 
    ...
    * adjust for weights 
    ...
    gen DW0_`prog' = mean_within_`prog' + mean_fc_`prog' + mean_LS_`prog' 
    ...
    * collapse to facilitate merge 
    keep if _n == 1
    gen i = 1
    keep mean_within* mean_fc* mean_ls* dw* weight_dollars* coeff* i 
...
* load results from all programs   
  ...
  foreach prog in wic medicaid liheap ha tanf ssi schoolmeals {
    ...
    foreach var in mean_within mean_within_neut mean_within_ins mean_fc mean_ls dw0 coeff {
  ...
  * export data only if entering a table 
   save "$dir/data/tmp/welfare_for_table_`weighttype'`nm'", replace
    ...
    use "$dir/data/tmp/welfare_for_table_`weighttype'`nm'" , clear
    
    cap file close fh
    file open fh using "$dir/figures/welfare_means_`weighttype'`nm'.csv", write replace
      
    foreach var of varlist mean_within* mean_fc* mean_ls* dw* {
      sum `var'
      file write fh "`var'`suffix'," %5.1f (`=`r(mean)'*100') _n 
  }
...
/****************************************/
/* * prepare data with welfare weights  */
/****************************************/
cap pr drop prep_welfare
pr define prep_welfare
...
*/
    ...
    * export
        ...
        preserve 
          ...
          save "$dir/data/tmp/emp_welfare_`weighttype'`nm'_`prog'", replace
        restore 
        }
