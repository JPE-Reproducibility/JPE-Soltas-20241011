...
quietly infix              ///
  ...
  int     age       84-86  ///
  long    ftotinc   87-93  ///
  using `"$dir/data/acs/usa_00072.dat"'
...
***************** Estimation code for local Pareto parameter *****************

* Drop is missing income
replace ftotinc = . if ftotinc == 9999999
drop if missing(ftotinc)

* Replace negative income to zero
replace ftotinc = 0 if ftotinc < 0

* Compute percentiles of distribution
gquantiles p_ftotinc = ftotinc [aw=hhwt], pctile nquantiles(100) genp(pctile_ftotinc)

* Add max of distribution, but will impute Pareto parameter there
replace pctile_ftotinc = 100 if pctile_ftotinc[_n-1] == 99
quietly summ ftotinc

* Income at bottom percentile is zero --> use income at half of 2nd percentile instead
* for density estimation
replace p_ftotinc = p_ftotinc[_n+1]/2 if p_ftotinc==0
gen p_lftotinc = ln(p_ftotinc)

* Estimate density in log terms using bandwidth of 0.3 in log income
gen lftotinc = ln(ftotinc)
kdens lftotinc [aw=hhwt], bw(0.3) gen(dens) at(p_lftotinc) nograph ll(0)

* Get local Pareto parameter
gen pareto = -((dens[_n+1]-dens[_n-1])/(p_lftotinc[_n+1]-p_lftotinc[_n-1]))/dens

* Impute bottom and top values based on surrounding vals
keep pctile_ftotinc  p_lftotinc pareto
drop if missing(pctile_ftotinc)
replace pareto = pareto[_n+1] if pctile_ftotinc == 1
replace pareto = pareto[_n-1] if pctile_ftotinc == 99
replace pareto = pareto[_n-1] if pctile_ftotinc == 100

* Clean up data file
keep pctile_ftotinc p_lftotinc pareto 
keep if !missing(pctile_ftotinc)
drop p_lftotinc

sort pctile_ftotinc pareto
order pctile_ftotinc pareto

* Recenter estimates (to zero)
summ pareto
replace pareto = pareto - r(mean)

* Save to file
save "$dir/data/acs/pareto_param.dta", replace

* Save to figure
tw line pareto pct, ///
	xtitle("Current Income Percentile") ytitle("Local Pareto Parameter") ///
	ylabel(,nogrid) graphregion(color(white))

gr export "$dir/figures/pareto_param.pdf", replace
