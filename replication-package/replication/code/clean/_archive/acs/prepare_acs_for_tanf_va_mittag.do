...
quietly infix                ///
  ...
  byte    hispan    97-97    ///
  int     hispand   98-100   ///
  byte    citizen   101-101  ///
  byte    educ      102-103  ///
  int     educd     104-106  ///
  long    incwage   107-112  ///
  long    incwelfr  113-117  ///
  long    incsupp   118-122  ///
  using `"$dir/data/acs/usa_00073.dat"'
...
*** Steps to align ACS with SIPP data variables

	* Compute SSI participation
	gen ssi = incsupp > 0 if !missing(incsupp)
	
	* Recode sex
	gen female = sex == 2 if !missing(sex)
	
	* Recode education
	gen ed_lths = educd <= 61 if !missing(educd)
	gen ed_hsdeg = educd == 62 | educd == 63 | educd == 64
	gen ed_scoll = educd > 64 & educd < 100
	gen ed_ba = educd == 100
	gen ed_mtcoll = educd > 100 if !missing(educd)
	
	* Recode marital status
	gen marst_single = marst == 6
	gen marst_married = marst == 1 | marst == 2
	gen marst_separated = marst == 3
	gen marst_divorced = marst == 4
	gen marst_widowed = marst == 5
	
	* Recode racial/ethnic identification
	gen race_hispanic = hispan != 0 if !missing(hispan)
	gen race_white = (race == 1)*(race_hispanic==0)
	gen race_black = (race == 2)*(race_hispanic==0)
	gen race_aian = (race == 3)*(race_hispanic==0)
	gen race_asian = (race == 4 | race == 5 | race == 6)*(race_hispanic==0)
	gen race_oth = (race == 7 | race == 8 | race == 9)*(race_hispanic==0)
	
	* Recode citizenship status
	gen citizen_ = citizen == 0 | citizen == 1 | citizen == 2 if !missing(citizen)
	drop citizen
	rename citizen_ citizen

	* Recode earnings data 
	gen earnings0 = incwage == 0
	gen earnings0_1000 = incwage > 0 & incwage < 1000
	gen earnings1000_2500 = incwage >= 1000 & incwage < 2500
	gen earnings2500_5000 = incwage >= 2500 & incwage < 5000
	gen earnings7500_10000 = incwage >= 7500 & incwage < 10000
	gen earnings10000_15000 = incwage >= 10000 & incwage < 15000
	gen earnings15000_20000 = incwage >= 15000 & incwage < 20000
	gen earnings20000_30000 = incwage >= 20000 & incwage < 30000
	gen earnings30000_40000 = incwage >= 30000 & incwage < 40000
	gen earnings40000_50000 = incwage >= 40000 & incwage < 50000
	gen earnings50000_up = incwage >= 50000
	
*** Collapse to weighted counts
	
	replace perwt = perwt
	
	collapse (sum) perwt if ssi == 1, by(year age female race_* ed_* marst_* earnings* citizen)
