* ebayes fe se [vars], gen() bee() theta() var() uvar() rawvar() by() absorb() simple

* fe - the fixed effects
* se - their standard errors (not variance!)
* vars - RHS vars in the regression used to estimate the true mean
*        of the fe (theta). can be left blank... probably should be.
* gen() - variable to put empirical bayes adjusted fe
* bee() - attenuation factor ("B")
* theta() - value toward which the fe was attenuated (e.g. mu)
* var() - underlying variance of the fe-DGP (conditional on the RHS vars)
* uvar() - unconditional underlying variance of the fe-DGP
* rawvar() - variance of the estimated FE (using same weights as var() and uvar())
* by() - perform empirical bayes procedure at this level
* absorb() - attenuate toward the mean at this level
* simple - use constant weights to estimate underlying FE and attenuation factor
* tol() - tolerance for convergence (default .0001)
* maxiter() - maximum # of iterations (default 20)
...
program define ebayes, sortpreserve
	...
	gettoken fe varlist : varlist
	gettoken se varlist : varlist
	local coeff "`varlist'"
